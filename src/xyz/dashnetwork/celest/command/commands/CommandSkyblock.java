...
public final class CommandSkyblock extends CelestCommand {
    ...
    public void execute(CommandSource source, String label, Arguments arguments) {
        Optional<RegisteredServer> optional = proxy.getServer("skyblock");
        List<Player> players = arguments.playerListOrSelf(source);

        if (optional.isEmpty()) {
            MessageUtil.message(source, "&6&l»&7 No server for &6Skyblock&7 was found.");
            return;
        }
        ...
        for (Player player : players) {
            ...
            player.createConnectionRequest(server).connect().thenAccept(u -> {
                if (LazyUtil.anyEquals(u.getStatus(),
                    ...
                    Section section = kick.append("&6&l»&7 Failed to connect to &6" + name + "&7. Hover for more info.");
