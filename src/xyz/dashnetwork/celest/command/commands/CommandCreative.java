...
public final class CommandCreative extends CelestCommand {
    ...
    public void execute(CommandSource source, String label, Arguments arguments) {
        Optional<RegisteredServer> optional = proxy.getServer("creative");
        List<Player> players = arguments.playerListOrSelf(source);

        if (optional.isEmpty()) {
            MessageUtils.message(source, "&6&l»&7 No server for &6Creative&7 was found.");
            return;
        }
