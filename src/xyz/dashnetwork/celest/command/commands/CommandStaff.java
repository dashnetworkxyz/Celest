...
public final class CommandStaff extends CelestCommand {
    ...
    public void execute(CommandSource source, String label, Arguments arguments) {
        Optional<RegisteredServer> optional = proxy.getServer("staff");
        List<Player> players = arguments.playerListOrSelf(source);

        if (optional.isEmpty()) {
            MessageUtil.message(source, "&6&l»&7 No server for &6Staff&7 was found.");
            return;
        }
