...
public final class CommandStafflegacy extends CelestCommand {

    private static final ProxyServer proxy = Celest.getServer();

    public CommandStafflegacy() {
        super("stafflegacy");
    ...
    public void execute(CommandSource source, String label, Arguments arguments) {
        Optional<RegisteredServer> optional = proxy.getServer("stafflegacy");
        List<Player> players = arguments.playerListOrSelf(source);

        if (optional.isEmpty()) {
            MessageUtils.message(source, "&6&l»&7 No server for &6Stafflegacy&7 was found.");
            return;
        }
