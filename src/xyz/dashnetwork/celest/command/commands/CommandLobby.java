...
public final class CommandLobby extends CelestCommand {
    ...
    public void execute(CommandSource source, String label, Arguments arguments) {
        Optional<RegisteredServer> optional = proxy.getServer("lobby");
        List<Player> players = arguments.playerListOrSelf(source);

        if (optional.isEmpty()) {
            MessageUtils.message(source, "&6&l»&7 No server for &6Lobby&7 was found.");
            return;
        }
